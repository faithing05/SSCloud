...
Разобрал train/ablations/ablation_with_6_classes/summary/summary_run_ablation_2.md.
Что получилось по ablation 2:
- A_Baseline_ref_s010: Dice 0.4630, mIoU 0.3943, time 0.0098
- B_Sampler_balanced_s010: Dice 0.4494, mIoU 0.3824, time 0.0093
- C_Hybrid_mild_s010: Dice 0.5080, mIoU 0.3873, time 0.0087
- D_Weights_mild_s010: Dice 0.3842, mIoU 0.3881, time 0.0091
Ключевые выводы:
- Лучший по главной метрике Dice — C_Hybrid_mild_s010 (0.5080), и он же самый быстрый по инференсу.
- Лучший mIoU — A (0.3943), но отрыв от C небольшой (+0.0070), а по Dice A заметно хуже (-0.045).
- B не дал пользы относительно A/C.
- D явно слабый по Dice, его стоит исключить из следующих прогонов.
- Редкие классы 4/5 по-прежнему почти нулевые у всех конфигов (системная проблема, не решена в этом цикле).
Практическое решение:
- Текущий кандидат-референс для следующей итерации: C_Hybrid_mild_s010.
- Для ablation 3 имеет смысл делать только варианты вокруг C (очень мягкий тюнинг sampler/weights), без scale-up и без агрессивных весов.  
Если хочешь, сразу напишу run_ablation_3.sh в таком формате (4 безопасных эксперимента вокруг C).