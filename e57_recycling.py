...
def save_points_to_laz(points, output_path):
    ...
    # laspy ожидает целочисленные координаты (int64)
    # Координаты уже должны быть умножены на 1/scale
    las.X = points['X']
    las.Y = points['Y']
    las.Z = points['Z']
    las.intensity = points['intensity'].astype(np.uint16)
    # В LAS 1.4 цвет хранится в 16-битном формате.
    # Для преобразования из 8-бит (0-255) в 16-бит (0-65535) нужно умножить на 256.
...
def process_e57_file(e57_path):
    """
    Функция для полной обработки одного файла .e57.
    Читает облако точек, вычисляет нормали, сохраняет LAZ и JPG.
    """
    print(f"\n--- Начало обработки файла: {os.path.basename(e57_path)} ---")
    ...
    try:
        ...
        print(f"Позиция скана (sensor origin): {sensor_origin}")
        ...
        if len(X) == 0:
            print("В скане нет точек. Пропускаем файл.")
            return
        ...
        print(f"JPG нормалей сохранён: {img_norm_out}")
        ...
        if n_pts == 0:
            print("Нет внутренних точек для сохранения в LAZ. Пропускаем.")
            return
        ...
        # Для записи в laspy координаты должны быть целочисленными
        scale = 0.000001
        ...
        # Интенсивность нужно перевести в 2D массив для индексации
        ...
        print(f"!!! Произошла ошибка при обработке файла {os.path.basename(e57_path)}: {e}")

    finally:
        print(f"Обработка файла заняла: {time.perf_counter() - t0:.3f} с")
