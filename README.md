...
2.  **Подготовьте панорамы**: Убедитесь, что ваши `.jpg` файлы панорам находятся в папке `Data_Input/`.
...
```powershell
docker run --gpus all -p 8000:8000 ` -v "${PWD}:/workspace/SSCloud" keiretsu/sscloud-service:v1 
```
...
*   `-v ...`: связывает папку на вашем компьютере (слева от `:`) с папкой внутри контейнера (справа от `:`). Это позволяет приложению читать ваши панорамы и сохранять результаты (`Data_Output`) прямо на ваш компьютер.
...
2.  Вы увидите простой интерфейс. В поле "Имя файла панорамы" введите точное имя файла, который находится в папке `Data_Input` (например, `3_normals.jpg`).
...
5.  После классификации всех масок результаты будут сохранены в папке `Data_Output/<имя_панорамы>/`. Вы можете добавить в интерфейс кнопку для вызова эндпоинта, который будет собирать финальный ZIP-архив для CVAT.
