...
Этот раздел описывает, как запустить интерактивное веб-приложение для разметки панорам с использованием модели Segment Anything (SAM). Этот воркфлоу заменяет старый подход с Jupyter Notebook.
...
Рекомендуется использовать **абсолютные пути**, чтобы избежать ошибок. Замените `F:\Desktop\SSCloud` на ваш реальный путь к проекту.
